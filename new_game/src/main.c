...
/*   Updated: 2025/08/05 17:18:53 by silpaukn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_game(t_game *game)
{
	...
	game->mlx = mlx_init();
	game->north.ptr = mlx_xpm_file_to_image(game->mlx, game->no_path, &game->north.width, &game->north.height);
	if (game->north.ptr)
		game->north.addr = (int *)mlx_get_data_addr(game->north.ptr, &game->north.bitsinpixel, &game->north.line_bytes, &game->north.endian);
	game->east.ptr = mlx_xpm_file_to_image(game->mlx, game->ea_path, &game->east.width, &game->east.height);
	if (game->east.ptr)
		game->east.addr = (int *)mlx_get_data_addr(game->east.ptr, &game->east.bitsinpixel, &game->east.line_bytes, &game->east.endian);
	game->south.ptr = mlx_xpm_file_to_image(game->mlx, game->so_path, &game->south.width, &game->south.height);
	if (game->south.ptr)
		game->south.addr = (int *)mlx_get_data_addr(game->south.ptr, &game->south.bitsinpixel, &game->south.line_bytes, &game->south.endian);
	game->west.ptr = mlx_xpm_file_to_image(game->mlx, game->we_path, &game->west.width, &game->west.height);
	if (game->west.ptr)
...
// 			if (game->map[y][x] == '1')
// 				if (DEBUG)
// 					draw_square(x * BLOCK_SIZE,
// 						y * BLOCK_SIZE, BLOCK_SIZE, 0xFF0000, game);
// 			x++;
// 		}
// 		y++;
// 	}
...
// 	else
// 		texture = &game->west;
// 	step = 1.0 * TEXTURE_WIDTH / line_height;
